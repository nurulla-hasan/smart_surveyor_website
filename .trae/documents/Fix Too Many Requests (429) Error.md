...
- **Update `src/app/(dashboard)/bookings/page.tsx`**:
    ...
    - **Optimize Redundant Requests**: If the active tab is "pending", I will skip the extra `getBookings({ filter: "pending", pageSize: "1" })` call and instead use the total items count from the main bookings response.
    - **Sequential Fetching**:
